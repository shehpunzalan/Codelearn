...
function AppContent() {
  console.log(
    "🚀 AppContent rendering - timestamp:",
  ...
  const [selectedLessonId, setSelectedLessonId] = useState<
    string | null
  >(null);
  const [selectedLessonTitle, setSelectedLessonTitle] =
    useState<string>("");
      ...
      for (let i = localStorage.length - 1; i >= 0; i--) {
        ...
        if (key?.startsWith("completedLessons_mod1")) {
          try {
            ...
            const hasRealQuiz1 = !!localStorage.getItem(
              "quiz_mod1_lesson1-1",
            );
            const hasRealQuiz2 = !!localStorage.getItem(
              "quiz_mod1_lesson1-2",
            );
            ...
            if (cleaned.length !== arr.length) {
              ...
              if (mod) {
                const pct = Math.round(
                  (cleaned.length / mod.totalLessons) * 100,
                );
                localStorage.setItem(
                  "moduleProgress_mod1",
                  JSON.stringify({
                    progress: pct,
                    completedLessons: cleaned.length,
                  }),
                );
              }
            }
    ...
    const checkSession = async () => {
      ...
      try {
        ...
        if (
          ...
          accessToken === "demo-token-instructor"
        ) {
          if (currentUser) {
  ...
  const handleLogin = async (loggedInUser: User) => {
    ...
    if (
      ...
      try {
        const res = await backendApi.getUserPosition(
          loggedInUser.id,
        );
        ...
        /* silently ignore – backend may not have a saved position */
      }
    }
  };
  ...
  const handleLessonComplete = (
    ...
    setModules((prev) =>
      prev.map((m) =>
        m.id === moduleId
          ? { ...m, progress, completedLessons: completedCount }
          : m,
      ),
    );
  ...
  const syncUserStats = (userId: string) => {
    ...
    const completedModules = allModuleProgress
      .filter((m) => m.progress === 100)
      .map((m) => m.id);
    const totalLessonsCompleted = allModuleProgress.reduce(
      (s, m) => s + (m.completedLessons || 0),
      0,
    );
  ...
  const handleOpenReadingContent = (
    moduleId: string,
    lessonId: string,
    lessonTitle: string,
    lessonContent: any,
  ...
  const handleOpenInteractiveGame = (
    moduleId: string,
    lessonId: string,
    lessonTitle: string,
    lessonContent: any,
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-blue-50/50 via-white to-purple-50/50">
      {resumePrompt && (
        ...
        >
          <div style={{ flex: 1 }}>
            ...
            >
              {resumePrompt.moduleTitle} &rsaquo;{" "}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {currentView === "learning-path-reading" &&
          selectedModuleId &&
          selectedLessonId && (
        ...
        {![
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 mb-6">
              The view "{currentView}" doesn't exist.
            </p>
            <button
        ...
        >
          ...
          <p style={{ color: "#4b5563", marginBottom: "1rem" }}>
            {error instanceof Error
              ? error.message
              : "An unknown error occurred while loading CodeLearn AI"}
          </p>
